...
class FunctionProfiler(Decorator):
    """
    ``FunctionProfiler`` is a :class:`pydeco.Decorator` that profile the execution of a function such as execution time, memory usage etc.
    
    Several profiler utils :class:`pydeco.decorators.ProfilerUtils` can be connected to the ``FunctionProfiler`` according to the user's needs.
    Each profiler utils is used to collect a specific data during the function execution.
    For example, the :class:`pydeco.decorators.Timer` is used to collect the execution time of the function and the :class:`pydeco.decorators.Memory` is used to collect the memory usage of the function.
    ...
    The `signature_name_format` attribute of this decorator can be used to customize the function signature name (see :class:`pydeco.Decorator`).
    ...
    report_format : str
        The format of the string to report the profiled data. (see :meth:`pydeco.decorators.function_profiler.FunctionProfiler.set_report_format`).
    ...
    Methods
    -------
    connect_profiler_utils(profiler_utils: Union[Type, List[Type]])
        Connects a sub-class of the ``ProfilerUtils`` class to the ``FunctionProfiler``. If a logger utils is None, it will be ignored. 

    disconnect_all()
        Disconnects all the ``ProfilerUtils`` to the ``FunctionProfiler`` and initializes the profiled data.

    initialize()
        Initializes the ``FunctionProfiler`` by removing all the profiled data.

    set_report_format(report_format: str)
        Sets the format of the string to report the profiled data.
    
    get_report_format() -> str
        Gets the format of the string to report the profiled data.

    extract_profiled_functions() -> List[Callable]
        Extracts the list containing the functions/methods actually profiled by the ``FunctionProfiler``.
    
    extract_profiled_functions_signature_name() -> List[str]
        Extracts the list containing the signature name of the functions/methods actually profiled by the ``FunctionProfiler``.

    extract_connected_profiler_utils() -> List[ProfilerUtils]
        Extracts the list of the connected ``ProfilerUtils`` to the ``FunctionProfiler``.
    
    extract_profiled_data() -> List[List]
    ...
    extract_profiled_data_reorganized_by_function() -> Dict[int, List[List]]
        Reorganizes the profiled data by function.

    generate_report() -> str
        Generates the string reporting the profiled data according to the selected ``report_format``.

    generate_report_datetime() -> str
        Generates the string reporting the profiled data in the "datetime" format.

    generate_report_function() -> str
        Generates the string reporting the profiled data in the "function" format.

    generate_report_cumulative() -> str
        Generates the string reporting the profiled data in the "cumulative" format.

    write_report(file_path: str)
    ...
    def generate_report_datetime(self) -> str:
        ...
        .. seealso::
            ...
            :func:`pydeco.decorators.FunctionProfiler.set_report_format()`
        """
        report = ""
        for log in self._profiled_data:
            # Extract the data
